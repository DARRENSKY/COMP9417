...
def knn_regression(training_data, inX, labels, k):
    ...
    top_k_distances = distance[sorted_index[:k]]
    ret = 0
    denominator = 0
    for i in range(k):
        ret+=weight(top_k_distances.iloc[i])*top_k_labels.iloc[i]
        denominator+=weight(top_k_distances.iloc[i])
    return ret/denominator

def cross_validation_wnn_prediction_extra_features(k):
...
if __name__ == '__main__':
    k = 10
    cross_validation_wnn_prediction_extra_features(k)
